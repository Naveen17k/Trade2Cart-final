...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white p-4 shadow-md transition-all duration-300 ease-in-out">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="md:hidden" onClick={toggleMenu}>
            {menuOpen ? <FiX className="text-black w-6 h-6 transition transform duration-300 rotate-180" /> : <FiMenu className="text-black w-6 h-6 transition transform duration-300 rotate-0" />}
          </div>
        </div>
      </div>
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden bg-gray-50 shadow-md transition-all duration-500 ease-in-out transform ${menuOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}
      >
